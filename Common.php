...
//Connection Settings @0-F838E269
...
//Initialize Common Variables @0-C0101016
...
//Connection1 Connection Class @-24072949
...
//CCLoginUser @0-64F98BCC
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBConnection1();
    $SQL = "SELECT iduser, idgrupo, password FROM usuarios WHERE [user]=" . $db->ToSQL($login, ccsText) . " AND password=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("UserID", $db->f("iduser"));
        CCSetSession("UserLogin", $login);
        CCSetSession("GroupID", $db->f("idgrupo"));
    }
    return $Result;
}
//End CCLoginUser
