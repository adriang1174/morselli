...
$liquidaciones = explode(";", $liquida);
list($ano1, $mes1, $idalquiler) = explode(":", $liquidaciones[0]);
//$ficha = $_REQUEST['ficha'];
$ficha = $idalquiler;
$idhipoteca = $_REQUEST['idhipoteca'];

$conn = new clsDBConnection1();
$conn->query("EXEC sp_liquida_cuotas_hipoteca ".$nroliq.",".$idhipoteca.",'".$liquida."'"); 
$total = 0;
$honorarios = 0;
$detalle = array();
while($conn->next_record()){
	...
	{	
		$linea = array('Año' => $conn->f('ano'),'Mes' => $conn->f('mes'),'Importe'=>number_format($conn->f('importe'),2,'.',''));//
		$total += $linea['Importe'];
...
$sql = "select f.nombre,p.direccion,p.localidad,h.idhipoteca,m.descripcion,m.simbolo
from fichas f
inner join fichashipotecas fh on fh.idficha = f.idficha
inner join hipotecas h on h.idhipoteca = fh.idhipoteca
inner join propiedades p on p.idpropiedad = h.idpropiedad
inner join monedas m on h.idmoneda = m.idmoneda
where
f.idficha = ".$ficha." and h.idhipoteca = ". $idhipoteca;
//var_dump($sql);
$conn->query($sql);
$conn->next_record();
...
$pdf->Cell(35,10,'Recibí  de la firma ');
$pdf->Cell(50,10,$firma);
$pdf->Cell(80,10,'la cantidad de '.$moneda);
$pdf->Ln();
$pdf->Cell(100,10,$total_letras." ---------------------------------------------------------");
$pdf->Ln();
$pdf->Cell(120,10,'por el préstamo N² '.$idhipoteca. ' sobre la propiedad puesta en garantía ubicada en' );
$pdf->Ln();
...
$pdf->Cell(0,12,'Error al generar liquidación',0,0,'L');
}
?>