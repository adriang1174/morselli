<?php
//Include Common Files @1-F1AB0C90
define("RelativePath", "..");
define("PathToCurrentPage", "/caja/");
define("FileName", "aperturacaja.php");
...
//Class_Initialize Event @3-04AFEE11
    function clsRecordcajaresumen($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->saldoinicialdolar = & new clsControl(ccsTextBox, "saldoinicialdolar", "saldoinicialdolar", ccsFloat, "", CCGetRequestParam("saldoinicialdolar", $Method, NULL), $this);
        }
    }
...
//InsertRow Method @3-0EEFCAD0
    function InsertRow()
    {
        ...
        $this->DataSource->saldoinicialdolar->SetValue($this->saldoinicialdolar->GetValue(true));
...
//DataSource Variables @3-AA30EAD4
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @3-25E57140
    function clscajaresumenDataSource(& $Parent)
    {
        ...
        $this->saldoinicialdolar = new clsField("saldoinicialdolar", ccsFloat, "");
        

        $this->InsertFields["saldoinicial"] = array("Name" => "saldoinicial", "Value" => "", "DataType" => ccsFloat, "OmitIfEmpty" => 1);
        $this->InsertFields["fecha"] = array("Name" => "fecha", "Value" => "", "DataType" => ccsDate, "OmitIfEmpty" => 1);
        $this->InsertFields["saldoinicialdolar"] = array("Name" => "saldoinicialdolar", "Value" => "", "DataType" => ccsFloat, "OmitIfEmpty" => 1);
    }
//End DataSourceClass_Initialize Event

//Prepare Method @3-137D6DD3
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
...
//Insert Method @3-7B5D721B
    function Insert()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->CmdExecution = true;
        $this->cp["saldoinicial"] = new clsSQLParameter("ctrlsaldoinicial", ccsFloat, "", "", $this->saldoinicial->GetValue(true), NULL, false, $this->ErrorBlock);
        $this->cp["fecha"] = new clsSQLParameter("ctrlfecha", ccsDate, array("dd", "/", "mm", "/", "yyyy"), array("yyyy", "-", "mm", "-", "dd", " ", "HH", ":", "nn", ":", "ss"), $this->fecha->GetValue(true), NULL, false, $this->ErrorBlock);
        $this->cp["saldoinicialdolar"] = new clsSQLParameter("ctrlsaldoinicialdolar", ccsFloat, "", "", $this->saldoinicialdolar->GetValue(true), NULL, false, $this->ErrorBlock);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildInsert", $this->Parent);
        if (!is_null($this->cp["saldoinicial"]->GetValue()) and !strlen($this->cp["saldoinicial"]->GetText()) and !is_bool($this->cp["saldoinicial"]->GetValue())) 
            $this->cp["saldoinicial"]->SetValue($this->saldoinicial->GetValue(true));
        ...
        if (!is_null($this->cp["saldoinicialdolar"]->GetValue()) and !strlen($this->cp["saldoinicialdolar"]->GetText()) and !is_bool($this->cp["saldoinicialdolar"]->GetValue())) 
            $this->cp["saldoinicialdolar"]->SetValue($this->saldoinicialdolar->GetValue(true));
        $this->InsertFields["saldoinicial"]["Value"] = $this->cp["saldoinicial"]->GetDBValue(true);
        $this->InsertFields["fecha"]["Value"] = $this->cp["fecha"]->GetDBValue(true);
        $this->InsertFields["saldoinicialdolar"]["Value"] = $this->cp["saldoinicialdolar"]->GetDBValue(true);
        $this->SQL = CCBuildInsert("cajaresumen", $this->InsertFields, $this);
...
//Update Method @3-722D987F
    function Update()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->CmdExecution = true;
        $this->cp["saldoinicial"] = new clsSQLParameter("ctrlsaldoinicial", ccsFloat, "", "", $this->saldoinicial->GetValue(true), "", false, $this->ErrorBlock);
        ...
        $this->cp["saldoinicialdolar"] = new clsSQLParameter("ctrlsaldoinicialdolar", ccsInteger, "", "", $this->saldoinicialdolar->GetValue(true), 0, false, $this->ErrorBlock);
        ...
        if (!is_null($this->cp["saldoinicialdolar"]->GetValue()) and !strlen($this->cp["saldoinicialdolar"]->GetText()) and !is_bool($this->cp["saldoinicialdolar"]->GetValue())) 
            $this->cp["saldoinicialdolar"]->SetValue($this->saldoinicialdolar->GetValue(true));
        if (!strlen($this->cp["saldoinicialdolar"]->GetText()) and !is_bool($this->cp["saldoinicialdolar"]->GetValue(true))) 
            $this->cp["saldoinicialdolar"]->SetText(0);
        $this->SQL = "UPDATE cajaresumen \n" .
        "SET saldoinicial=" . $this->SQLValue($this->cp["saldoinicial"]->GetDBValue(), ccsFloat) . ", \n" .
        "saldoinicialdolar = " . $this->SQLValue($this->cp["saldoinicialdolar"]->GetDBValue(), ccsInteger) . "\n" .
...
//Delete Method @3-B498B2DE
    function Delete()
    {
        ...
        if (!strlen($this->cp["fecha"]->GetText()) and !is_bool($this->cp["fecha"]->GetValue(true))) 
            $this->cp["fecha"]->SetText(01/01/1960);
        $this->SQL = "delete from cajaresumen\n" .
        "where convert(varchar,fecha,103) = convert(varchar,'" . $this->SQLValue($this->cp["fecha"]->GetDBValue(), ccsDate) . "',103 )";
...
class clsRecordcajaresumen1 { //cajaresumen1 Class @23-0A3190B5
...
//Class_Initialize Event @23-CBA26072
...
//CheckErrors Method @23-2FF60E2A
    function CheckErrors()
    {
        $errors = false;
...
//Operation Method @23-CA4D02CA
    function Operation()
    {
        ...
        $Redirect = "aperturacaja.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "aperturacaja.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
} //End cajaresumen1 Class @23-FCB6E20C



//Initialize Page @1-73DFAE58
// Variables
$FileName = "";
$Redirect = "";
$Tpl = "";
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "aperturacaja.html";
$BlockToParse = "main";
...
//Include events file @1-058CCE10
include_once("./aperturacaja_events.php");
